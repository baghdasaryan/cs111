...
{
	int i, j;
	for (i = ntickets - 1; i >= 0; i--) {
		if (tickets[i] == pid) {
			for (j = i; j < ntickets - 1; j++) {
...
start(void)
{
	...
	for (i = 1; i < NPROCS; i++) {
		...
		#if PART_ON == 1
		proc->p_priority = 0;
		proc->p_proportional_share = 1;
		proc->p_running_time = 0;
		#endif
	}
	...
	scheduling_algorithm = 0; // 0, 1, 2, 3 (lottery)

	#if PART_ON == 1
	proc_array[1].p_running_time++;
	#endif
...
interrupt(registers_t *reg)
{
	...
	case INT_SYS_PROPORTIONAL_SHARE:
		current->p_proportional_share = reg->reg_eax;
		run(current);
	...
	case INT_SYS_TICKET:
		if(ntickets < MAX_TICKETS) {
			tickets[ntickets] = current->p_pid;
			ntickets += 1;
		}
		run(current);

	case INT_SYS_RM_TICKET:
...
schedule(void)
{
	...
	if (scheduling_algorithm == 0)
		...
		while (i < NPROCS || lowest_prop == 65535) {
			pid_t cur_pid = (i+pid) % NPROCS;
			float prop_value = (float) proc_array[cur_pid].p_running_time / 
					   (float) proc_array[cur_pid].p_proportional_share;
			if(proc_array[cur_pid].p_state == P_RUNNABLE 
		...
		proc_array[winner].p_running_time++;
		run(&proc_array[winner]);
	} else if (scheduling_algorithm == 3) {
		pid_t rnd_ticket;
		if (ntickets < NPROCS-1) {
			while (1) {
				pid = (pid + 1) % NPROCS;
				if (proc_array[pid].p_state == P_RUNNABLE)
					run(&proc_array[pid]);
			...
			do {
				rnd_ticket = tickets[(int) (Random() * 100000) % ntickets];
			} while (proc_array[rnd_ticket].p_state != P_RUNNABLE);
		}
		run(&proc_array[rnd_ticket]);
	}
	#endif
